    ...
    <div class="page-header-content header-elements-md-inline">
        <div class="page-title d-flex">
            <h4>
                ...
                <span class="font-weight-semibold">Estructura Monetaria</span>
            </h4>
        </div>
    </div>
</div>
...
<script>
    $('.datatable-ajax').dataTable({
        ajax: '{{ url('si/monedas/list') }}',
        columns: [
            {
                orderable: false,
                data: function(data) {
                    return '<input type="checkbox" name="row[]" value="'+data['id_moneda']+'">';
                }
            },
            { data: 'siglas_moneda' },
            { data: 'desc_moneda' },
            { 
                data: function(data) {
                    var idx  = data['tipo_moneda']
                    return moneda_tipo[idx];
                }
            },
            { 
                data: function(data) {
